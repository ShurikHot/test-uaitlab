...
@section('content')
    ...
    <div class="main" id="main">
        <div class="page-warranty-create">
            <form action="{{route('ate.update', $technicalConclusion->id)}}" method="post" class="card-lists js-form-validation" id="form-id">
                @csrf
                @method('PATCH')
            ...
            <div class="page-name sticky">
                ...
                <div class="btns">
                    <button type="submit" class="btn-border btn-blue text-only">Затвердити</button>
                            ...
                            <div class="card-content card-form">
                                ...
                                <div class="inputs-group one-row">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="number_1c" name="number_1c" value="{{$technicalConclusion->number_1c}}" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="date">Дата документу</label>
                                        <input type="text" id="date" name="date" value="{{$technicalConclusion->date}}" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="factory-number">Відповідальний</label>
                                        <input type="text" id="factory-number" value="{{$technicalConclusion->warrantyClaim->autor}}" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="barcode">Документ-підстава</label>
                                        <div class="input-wrapper">
                                            <input type="text" id="barcode" value="{{$technicalConclusion->warrantyClaim->number_1c}}" readonly>
                                            <a href="{{route('warranty.edit', $technicalConclusion->warrantyClaim->id)}}" target="_blank" class="icon-link-external text-blue"></a>
                                        </div>
                                    </div>
                                </div>
                                <div class="inputs-group one-row">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="field-1" value="{{$technicalConclusion->warrantyClaim->client_name}}" readonly>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="field-3" value="{{$technicalConclusion->warrantyClaim->product_article}}" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="field-4">Назва виробу</label>
                                        <input type="text" id="field-4" value="{{$technicalConclusion->warrantyClaim->product_name}}" readonly>
                                    </div>
                                </div>
                                <div class="inputs-group one-row">
                                    <div class="form-group">
                                        <label for="place-sale">Місце продажу</label>
                                        <input type="text" id="place-sale" value="{{$technicalConclusion->warrantyClaim->point_of_sale}}" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="date-sale">Дата продажу</label>
                                        <input type="text" id="date-sale" value="{{$technicalConclusion->warrantyClaim->date_of_sale}}" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="date-start">Дата звернення в сервісний центр</label>
                                        <input type="text" id="date-start" value="{{$technicalConclusion->warrantyClaim->date_of_claim}}" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="receipt-number">Номер квитанції сервісного центру</label>
                                        <input type="text" id="receipt-number" value="{{$technicalConclusion->warrantyClaim->receipt_number}}" readonly>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="card-content card-form">
                                    ...
                                    <div class="inputs-group one-row">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="field-5" value="{{$technicalConclusion->warrantyClaim->factory_number}}" readonly>
                                                        ...
                                                        @if(isset($parentDefectCode['children']))
                                                            @foreach($parentDefectCode['children'] as $childDefectCode)
                                                                <option class="" value="{{ $childDefectCode['code_1C'] }}"
                                                                        @if($technicalConclusion->defect_codes_code_1c == $childDefectCode['code_1C']) selected @endif>
                                                        ...
                                                        @if(isset($parentSymptomCode['children']))
                                                            @foreach($parentSymptomCode['children'] as $childSymptomCode)
                                                                <option class="" value="{{ $childSymptomCode['code_1C'] }}"
                                                                        @if($technicalConclusion->symptom_codes_code_1c == $childSymptomCode['code_1C']) selected @endif>
                                    ...
                                    <div class="inputs-group one-row">
                                        <div class="form-group">
                                            ...
                                            <textarea id="conclusion" name="conclusion" placeholder="Причина дефекту" rows="3">{{ old('conclusion', $technicalConclusion->conclusion ?? '') }}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <label for="resolution">Резолюція</label>
                                            <textarea id="resolution" name="resolution" placeholder="Причина дефекту" rows="3">{{ old('resolution', $technicalConclusion->resolution ?? '') }}</textarea>
                                        </div>
                                    </div>
