        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row d-flex justify-content-between mb-2">
                    ...
                    <a href="{{route('defect-codes.create')}}" class="btn btn-primary mr-3">Додати новий</a>
                </div>
            </div><!-- /.container-fluid -->
        </section>
                            ...
                            @foreach($tree as $item)
                                <tr class="bg-blue">
                                    ...
                                    <th class="justify-content-center d-flex">
                                        <a href="{{route('defect-codes.edit', $item['id'])}}"><button><i class="fas fa-pencil-alt"></i></button></a>
                                        <form action="{{route('defect-codes.destroy', $item['id'])}}" method="post">
                                            @csrf
                                            @method('DELETE')
                                ...
                                @if (isset($item['children']) && count($item['children']))
                                    ...
                                    <tr>
                                        ...
                                        <th class="justify-content-center d-flex">
                                            <a href="{{route('defect-codes.edit', $child['id'])}}"><button><i class="fas fa-pencil-alt"></i></button></a>
                                            <form action="{{route('defect-codes.destroy', $child['id'])}}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('Підтвердіть видалення')">
                                                    <i class="fas fa-trash"></i>
